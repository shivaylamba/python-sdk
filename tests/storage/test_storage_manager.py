...
def test_build_for_rdbms_postgresql_rollback(mock_config):
    ...
    mock_config.driver.schema.version.read.return_value = len(PostgresqlDriver.migrations)
...
def test_build_for_rdbms_no_migration_mapping(mock_config):
    ...
    assert "No migration mapping found for dialect: unknown_dialect" in str(exc_info.value)
